...
def upsert_to_postgres(df, epoch_id: int):
    ...
    rows = [
        ...
        # .collect() : spark DataFrame을 python List로 변환
        for r in df.collect()                         
    ]
    if not rows:
        return
...
if __name__ == "__main__":
    ...
    raw_df = (
        spark.readStream
        .format("kafka")
        .option("kafka.bootstrap.servers", KAFKA_BOOTSTRAP) # 브로커 주소
        .option("subscribe", KAFKA_TOPIC)                   # 구독 토픽
        .option("startingOffsets", "latest")                # 스트리밍 시작 이후 새로 들어오는 메시지부터
        .load()                                             # 스트림 로드
    )
